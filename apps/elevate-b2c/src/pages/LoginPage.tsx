...
export const LoginPage: React.FC<LoginPageProps> = ({ onGuestLogin }) => {
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-[#0A1628]/10 border border-[#E2E6EC] p-8">
            ...
            <div className="space-y-2.5">
              <button
                ...
                className="w-full py-3 rounded-xl border-2 border-[#0A1628] bg-[#0A1628] text-white hover:bg-[#1A2B45] text-sm font-bold flex items-center justify-center gap-2.5 transition shadow-sm cursor-pointer"
              >
                <Sparkles className="w-4 h-4 text-[#D4A017]" />
                <span>Entrar en Modo Prueba (Ver App Directo)</span>
              </button>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (error) {
                      setErrorMsg(`Google Auth no está activo aún en tu proyecto de Supabase. Puedes presionar "Entrar en Modo Prueba" arriba para navegar la app de inmediato.`)
                      setLoading(false)
                    }
                  } catch (err: any) {
                    setErrorMsg('Google Auth pendiente de activar en Supabase. Usa "Entrar en Modo Prueba" para explorar la app.')
                    setLoading(false)
                  }
                }}
                className="w-full py-2.5 rounded-xl border border-[#E2E6EC] bg-white hover:bg-[#F8F9FB] text-xs font-semibold text-[#0A1628] flex items-center justify-center gap-2.5 transition disabled:opacity-50 cursor-pointer"
              >
                ...
                <span>{loading ? 'Conectando...' : 'Continuar con Google (OAuth)'}</span>
              </button>
            </div>
